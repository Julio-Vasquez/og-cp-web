...
export const BarChart: FC<BarChartProps> = ({ selectedChild, selectedPhase }) => {
  ...
  const data: BarChartResponse =
    response.status === Status.success ? response.payload : ({} as BarChartResponse)
  ...
  const config = {
    ...
    yField: 'pctCompleted',
    style: {
      fill: (d: any) => (+d.pctCompleted >= 50 / 100 ? '#6744c6' : '#6744c6a6'),
    },
    label: {
      text: 'pctCompleted',
      formatter: '.0%',
      style: {
        textAnchor: (d: any) =>
          +d.pctCompleted > 1 || +d.pctCompleted === 100 / 100 ? 'right' : 'start',
        fill: (d: any) =>
          +d.pctCompleted > 1 || +d.pctCompleted === 100 / 100 ? '#fff' : '#000',
